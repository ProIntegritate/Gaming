...
cd %SERVERGAMENAME%
rmdir /s /q . >nul
7z x ..\backup.%SERVERGAMENAME%.zip >nul
cd ..
...
cd
rmdir /s /q . >nul
7z x ..\backup.%SERVERGAMENAME%.player.zip >nul

cd "%userprofile%\Zomboid\Server\"
echo * Restoring server configuration...
7z x %userprofile%\Zomboid\Saves\Multiplayer\backup.%SERVERGAMENAME%.serverconfig.zip -aoa >nul
